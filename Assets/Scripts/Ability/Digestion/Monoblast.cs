...
[CreateAssetMenu(fileName = "Monoblast", menuName = "Scriptable/Ability/Digestion/Monoblast")]
public class Monoblast : Digestion
{
    public override void Execute(object param0, object param1, object param2)
    {
        CreatureGroup group = (CreatureGroup)param0;
        MonoblastCountDown countDown = MonoblastCountDown.List.Find(i => (i.Group == group));
        ...
        if (countDown == null || countDown.CountDown == 2)
        {
            ...
            if (countDown == null)
            {
                MonoblastCountDown.List.Add(new MonoblastCountDown(group));
            }
            else
            {
    ...
    {
        return AbilityList.ABILITIES.MONOBLASTIC;
    }
}
